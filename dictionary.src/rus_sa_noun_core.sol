...
{
 w1=@regex_strict("[\\@А\\@Б\\@Й][\\@а\\@б\\@й]+"){ [-1]ПАДЕЖ:ИМ [-1]ЧИСЛО:ЕД } : export { node:root_node }
} : ngrams { -1 }
...
{
 @and(
      @regex_strict("[\\@А\\@БЙ][\\@а\\@б\\@й]+"){ [-1]ПАДЕЖ:ИМ [-1]ЧИСЛО:ЕД }:export { node:root_node },
...
pattern ГруппаСущ1
{
 ...
 n=Сущ0{ [-7]=Adj:ЧИСЛО [-7]=Adj:ПАДЕЖ [-7]=Adj:РОД [-7]=Adj:ОДУШ }:export{ РОД ПАДЕЖ ЧИСЛО ОДУШ МОДАЛЬНЫЙ CHARCASING ПАДЕЖВАЛ node:root_node }
}
: links { n.<ATTRIBUTE>adj }
: ngrams
{
...
pattern СчитаемыйНаречиемОбъект
{
 СущСРодДоп{ ПАДЕЖ:ПАРТ ~НеСчитабельныеСущ } : export { node:root_node }
}
...
pattern СчитаемыйНаречиемОбъект
{
 СущСРодДоп{ ПАДЕЖ:РОД ~НеСчитабельныеСущ } : export { node:root_node }
}
