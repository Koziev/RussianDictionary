...
patterns НазваниеОбъекта { language=Russian } export { node:root_node }
...
{
 adj=Суперлатив0
 n=Сущ0{ =Adj:ЧИСЛО =Adj:ПАДЕЖ =Adj:РОД =Adj:ОДУШ }:export{ РОД ПАДЕЖ ЧИСЛО ОДУШ МОДАЛЬНЫЙ CHARCASING ПАДЕЖВАЛ node:root_node }
}
...
{
 adj=Суперлатив0
 adj2=ГруппаПрил2{ =Adj:ЧИСЛО =Adj:ПАДЕЖ =Adj:РОД }
 n=Сущ0{ =Adj:ЧИСЛО =Adj:ПАДЕЖ =Adj:РОД =Adj:ОДУШ }:export{ РОД ПАДЕЖ ЧИСЛО ОДУШ МОДАЛЬНЫЙ CHARCASING ПАДЕЖВАЛ node:root_node }
}
: links
{
...
pattern ГруппаСущ1 export{ РОД ПАДЕЖ ЧИСЛО ОДУШ МОДАЛЬНЫЙ CHARCASING (ПАДЕЖВАЛ) node:root_node }
{
 adj=ГруппаПрил2{ ~RuAdjCannotModifNoun }
 q1=ОткрывающаяКавычка
 n=СущСПредложДоп{ =Adj:ЧИСЛО =Adj:ПАДЕЖ =Adj:РОД =Adj:ОДУШ }:export{ РОД ПАДЕЖ ЧИСЛО ОДУШ МОДАЛЬНЫЙ CHARCASING node:root_node }
 q2=ЗакрывающаяКавычка
} : links
{
 n.{
...
: ngrams { 1 }
 ...
 // частицы типа ДАЖЕ модифицируют существительное:
 wordentry_set ЧастМодифСущ={ частица:даже{}, наречие:только{}, частица:лишь{}, наречие:всего лишь{} }
...
pattern ГруппаСущ1
{
 ...
 n=Сущ0{ ПАДЕЖ:ПАРТ =Adj:ЧИСЛО =Adj:РОД =Adj:ОДУШ }:export{ РОД ПАДЕЖ ЧИСЛО ОДУШ МОДАЛЬНЫЙ CHARCASING ПАДЕЖВАЛ node:root_node }
} : links { n.<ATTRIBUTE>adj }
: ngrams
{
 adj_noun_score( adj, n )
...
pattern ГруппаСущ1
{
 ...
 n=Сущ0{ ПАДЕЖ:МЕСТ =Adj:ЧИСЛО =Adj:РОД =Adj:ОДУШ }:export{ РОД ПАДЕЖ:ПРЕДЛ ЧИСЛО ОДУШ МОДАЛЬНЫЙ CHARCASING ПАДЕЖВАЛ node:root_node }
} : links { n.<ATTRIBUTE>adj }
: ngrams
{
 adj_noun_score( adj, n )
...
wordentry_set НеРодДоп={ местоим_сущ:ничто{}, местоим_сущ:никто{}, местоим_сущ:себя{} }

patterns РодОбъект export { node:root_node }

pattern РодОбъектСущ
{
 РодОбъект{ ~НеРодДоп } : export { node:root_node }
}
...
pattern СущСРодДоп
{
 ...
 n2=существительное:*{ одуш:одуш CHARCASING:FirstCapitalized }
}
: ngrams { -1 }
: links { n1.<RIGHT_NAME>n2 }
...
} : links
{
 ...
 patterns ЭлементВНазвании export { node:root_node }
 pattern ЭлементВНазвании
 {
  *:*{ падеж:им } : export { node:root_node }
 }
 ...
 patterns ЦепочкаСловКакНазвание export { node:root_node }

 // Число жертв снежной бури "Немо" выросло до 9 человек
 //                           ^^^^
 pattern ЦепочкаСловКакНазвание
 {
  ЭлементВНазвании : export { node:root_node }
 }
 ...
 pattern ЦепочкаСловКакНазвание
 {
  head=ЭлементВНазвании : export { node:root_node }
 ...
 существительное:быль{}, // чтобы не возникало путаницы с формой глагола БЫТЬ
 существительное:прошлое{},
...
: links { n1.<RIGHT_INSTR_OBJECT>n2 }
...
{
 наречие:полно{}, // у него было полно работы
 наречие:столько{},
 наречие:сколько{},
 наречие:сколько-то{},
 наречие:столько-то{}, // Столько-то красок и времени - столько-то денег.
 наречие:несколько{},
 наречие:много{},
 наречие:достаточно{},
 ...
 наречие:чуточку{}, // дать тебе чуточку времени.
 наречие:мало{},
 наречие:маловато{},
 наречие:многовато{},
 наречие:немало{},
...
pattern НаречДляРод
{
 НаречДляРод0 : export { node:root_node }
}
...
pattern НаречДляРод
{
 a=НаречДляРод0 : export { node:root_node }
 p=частица:же{}
...
pattern НаречДляРод
{
 ...
 a=НаречДляРод0 { степень:атриб } : export { node:root_node }
} : links { a.<ATTRIBUTE>mod }
  : ngrams { 4 }
...
patterns ГруппаЧисл export { node:root_node ПАДЕЖ ЧИСЛО }
...
// несколько стаканов чаю стояло на столе
// ^^^^^^^^^^^^^^^^^^^^^^
pattern СущСРодДоп
{
 q=НаречДляРод
 n=СущСПредложДоп{ ПАДЕЖ:РОД ~НеСчитабельныеСущ } : export { ПАДЕЖ:ИМ ЧИСЛО:ЕД ОДУШ:НЕОДУШ РОД:СР node:root_node }
} : links { n.<ATTRIBUTE>q }
  : ngrams { 5 } // чтобы не дать наречию привязаться к сказуемому


// Счетный паттерн с прилагательным в роли именительного падежа:
// в карман,  где было много  таких  же.
//                     ^^^^^^^^^^^^^^^^
pattern СущСРодДоп
{
 q=НаречДляРод
 n=ГруппаПрил2{ ПАДЕЖ:РОД } : export { ПАДЕЖ:ИМ ЧИСЛО:ЕД ОДУШ:НЕОДУШ РОД:СР node:root_node }
} : links { n.<ATTRIBUTE>q }
  : ngrams { 1 }

// Счетный паттерн с прилагательным в роли винительного падежа:
// Мы взяли много пленных.
//          ^^^^^^^^^^^^^
pattern СущСРодДоп
{
 q=НаречДляРод
 n=ГруппаПрил2{ ПАДЕЖ:РОД } : export { ПАДЕЖ:ВИН ЧИСЛО:ЕД ОДУШ:НЕОДУШ РОД:СР node:root_node }
} : links { n.<ATTRIBUTE>q }
  : ngrams { 1 }
...
{
 q=НаречДляРод
 n=ГруппаПрил2{ ПАДЕЖ:РОД ЧИСЛО:ЕД РОД:СР } : export { ПАДЕЖ:ВИН ЧИСЛО:ЕД ОДУШ:НЕОДУШ РОД:СР node:root_node }
} : links { n.<ATTRIBUTE>q }
  : ngrams { 2 }
...
{
 q=НаречДляРод
 n=ГруппаПрил2{ ПАДЕЖ:РОД ЧИСЛО:ЕД РОД:СР } : export { ПАДЕЖ:ИМ ЧИСЛО:ЕД ОДУШ:НЕОДУШ РОД:СР node:root_node }
} : links { n.<ATTRIBUTE>q }
  : ngrams { 2 }
...
{
 q=НаречДляРод
 n=местоимение:*{ ПАДЕЖ:РОД } : export { ПАДЕЖ:ИМ ЧИСЛО:ЕД ОДУШ:НЕОДУШ РОД:СР node:root_node }
} : links { n.<ATTRIBUTE>q }
  : ngrams { 5 }
...
{
 q=НаречДляРод
 n=местоимение:*{ ПАДЕЖ:РОД } : export { ПАДЕЖ:ВИН ЧИСЛО:ЕД ОДУШ:НЕОДУШ РОД:СР node:root_node }
} : links { n.<ATTRIBUTE>q }
  : ngrams { 4 }
...
{
 q=наречие:еще{}
 n=СущСРодДоп{ ПАДЕЖ:РОД ~НеСчитабельныеСущ } : export { ПАДЕЖ:РОД ЧИСЛО ОДУШ РОД node:root_node }
} : links { n.<ATTRIBUTE>q }
  : ngrams { -1 }
...
{
 q=НаречДляРод
 n=СущСРодДоп{ ПАДЕЖ:РОД ~НеСчитабельныеСущ } : export { ПАДЕЖ:ВИН ЧИСЛО:ЕД ОДУШ:НЕОДУШ РОД:СР node:root_node }
} : links { n.<ATTRIBUTE>q }
  : ngrams { 5 } // чтобы не дать наречию привязаться к сказуемому
...
{
 q=НаречДляРод
 n=СущСРодДоп{ ПАДЕЖ:ПАРТ } : export { ПАДЕЖ:ВИН ЧИСЛО:МН ОДУШ:НЕОДУШ node:root_node }
} : links { n.<ATTRIBUTE>q }
...
pattern СущСПредложДоп
{
 ...
 prep=предлог:в{}
 n2=ГруппаСущ4 { падеж:им число:мн сущ_предл_сущ( N1.root_node, _.prepos, _.n2 ) }
}
: links { n1.<PREPOS_ADJUNCT>prep.<OBJECT>n2 }
: ngrams
{
 ngram3(n1,prep,n2)
 @tree_score(n1,ВалентностьСущ)
}
...
pattern СущСРодДоп
{
 // Запрещаем участие имён, то есть паттерны типы "Коля рыл погреб"
 n1=ГруппаСущ1{~CHARCASING:FirstCapitalized}:export{ РОД ПАДЕЖ ЧИСЛО ОДУШ node:root_node }
 n2=ГруппаСущ4{ ПАДЕЖ:РОД ~CannotBeGenAdjunct }
} : links { n1.<RIGHT_GENITIVE_OBJECT>n2 }
  : ngrams { сущ_сущ( n1.root_node, n2.root_node ) }
...
pattern СущСРодДоп
{
 // Запрещаем участие имён, то есть паттерны типы "Коля рыл погреб"
 n1=ГруппаСущ1{~CHARCASING:FirstCapitalized}:export{ РОД ПАДЕЖ ЧИСЛО ОДУШ node:root_node }
 n2=РодОбъектСущ //местоимение:я{ ПАДЕЖ:РОД }
} : links { n1.<RIGHT_GENITIVE_OBJECT>n2 }
 : ngrams { -4 }
...
pattern ПравоеПрилагательное
{
 ГруппаПрил2{ /*~ПостфиксПрилагАтрибут*/ /*~НеПостфиксПрилагАтрибут*/ } : export { node:root_node РОД ЧИСЛО ПАДЕЖ ОДУШ }
}
: ngrams { -5 }
...
/*
// Управляемый объект в дательном падеже.
// Может быть не только именем существительным:
pattern ДатОбъект
{
 ГлДополнение{ ПАДЕЖ:ДАТ } : export { node:root_node }
}
*/
...
pattern СущСРодДоп
{
 ...
 n2=ДатОбъект
} : links { n1.<RIGHT_DATIVE_OBJECT>n2 }
: ngrams
{
 -1
 сущ_сущ(n1,n2)
...
pattern СущСРодДоп
{
 ...
 n2=ДатОбъект
 inf=Инф2
} : links { n1.{ <RIGHT_DATIVE_OBJECT>n2 <INFINITIVE>inf } }
: ngrams
{
 -1
 сущ_сущ(n1,n2)
...
/*
// четкая команда собаке
pattern СущСРодДоп
{
 @mark(ГруппаПрил2,A)
 n1=СущУправлДат{ =A:РОД =A:ЧИСЛО =A:ПАДЕЖ =A:ОДУШ }:export{ РОД ПАДЕЖ ЧИСЛО ОДУШ node:root_node }
 n2=ГлДополнение{ ПАДЕЖ:ДАТ } 
} : links
{
 n1.{
     <ATTRIBUTE>a
...
/*
// Антонио посещал курсы обучения пению
pattern СущСРодДоп
{
 n1=ГруппаСущ1:export{ РОД ПАДЕЖ ЧИСЛО ОДУШ node:root_node }
 n2=СущУправлДат
 n3=ДатОбъект
} : links { n1.<RIGHT_DATIVE_OBJECT>n2.<RIGHT_GENITIVE_OBJECT>n3 }
: ngrams
{
 -1
 сущ_сущ(n1,n2)
...
/*
// Антонио посещал вечерние курсы обучения горловому пению
pattern СущСРодДоп
{
 n1=ГруппаСущ1:export{ РОД ПАДЕЖ ЧИСЛО ОДУШ node:root_node }
 @mark(ГруппаПрил2{ПАДЕЖ:РОД},A)
 n2=СущУправлДат{ =A:РОД =A:ЧИСЛО ПАДЕЖ:РОД =A:ОДУШ }
 n3=ДатОбъект
} : links
{
 n1.<RIGHT_GENITIVE_OBJECT>n2.{
...
wordentry_set СоединДляПридаточного={
 наречие:почему{}, наречие:зачем{}, союз:как{}, наречие:где{}, наречие:когда{},
 наречие:отчего{}, союз:что{}, союз:чтобы{}, союз:"с тем чтобы"{}
}
...
{
 /*существительное:*{}*/ СущСРодДоп : export { РОД ПАДЕЖ ЧИСЛО ОДУШ node:root_node }
}
...
pattern ТотКто_1
{
 ПрилДляТотКто : export{ РОД ПАДЕЖ ЧИСЛО ОДУШ:ОДУШ node:root_node }
}
...
: ngrams { 8 } // чтобы подавить вариант то=союз
...
: ngrams { 7 } // чтобы подавить вариант то=союз
