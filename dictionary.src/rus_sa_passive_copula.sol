...
/*
// Компаратив прилагательного может выступать в роли именного сказуемого:
//
// А, между прочим, наш мальчик не хуже.
//                              ^^^^^^^
pattern СтрадПрич0 export { РОД ЧИСЛО ПЕРЕХОДНОСТЬ (ПАДЕЖВАЛ) (МОДАЛЬНЫЙ) node:root_node }
{
 ГруппаСравнПрил : export { РОД:МУЖ ЧИСЛО:ЕД ПЕРЕХОДНОСТЬ:НЕПЕРЕХОДНЫЙ node:root_node }
} : ngrams { -2 }
*/
...
patterns СтрадСказВосх { bottomup } export { THEMA_VALENCY AUX_VERB_REQUIRED ЛИЦО ЧИСЛО РОД ПАДЕЖВАЛ ПЕРЕХОДНОСТЬ МОДАЛЬНЫЙ node:root_node }

pattern СтрадСказВосх export { THEMA_VALENCY AUX_VERB_REQUIRED (ЛИЦО) ЧИСЛО РОД ПАДЕЖВАЛ ПЕРЕХОДНОСТЬ МОДАЛЬНЫЙ node:root_node }
{
 СтрадПрич1 : export {
...
pattern СтрадСказВосх export { THEMA_VALENCY AUX_VERB_REQUIRED ЛИЦО ЧИСЛО РОД (ПАДЕЖВАЛ) (ПЕРЕХОДНОСТЬ) (МОДАЛЬНЫЙ) node:root_node }
{
 v=СтрадСказВосх : export { AUX_VERB_REQUIRED THEMA_VALENCY ЛИЦО ЧИСЛО РОД node:root_node }
 comma1=','
 conj=союз:что{}
 p=ПридаточноеВОбороте
 comma2=@coalesce(',')
...
{
 v=СтрадСказВосх{ THEMA_VALENCY:1 } : export { THEMA_VALENCY:0 AUX_VERB_REQUIRED ЛИЦО ЧИСЛО РОД ПАДЕЖВАЛ ПЕРЕХОДНОСТЬ МОДАЛЬНЫЙ node:root_node }
...
pattern СтрадСказВосх
{
 v=СтрадСказВосх{ ПЕРЕХОДНОСТЬ:ПЕРЕХОДНЫЙ } : export { node:root_node THEMA_VALENCY AUX_VERB_REQUIRED ЛИЦО ЧИСЛО РОД МОДАЛЬНЫЙ }
 obj=СтрадПричДополнение{ =ПАДЕЖ->v:ПАДЕЖВАЛ } : export {
                                                  @except(v:ПАДЕЖВАЛ,ПАДЕЖ)
                                                  @if_exported(ПЕРЕХОДНОСТЬ:ПЕРЕХОДНЫЙ,ПАДЕЖВАЛ)
                                                 }
...
pattern СтрадСказВосх
{
 v=СтрадСказВосх{ ~ПАДЕЖВАЛ:ДАТ } : export { node:root_node THEMA_VALENCY AUX_VERB_REQUIRED ЛИЦО ЧИСЛО РОД МОДАЛЬНЫЙ }
...
pattern СтрадСказВосх
{
 v=СтрадСказВосх{ ПричСВинДоп ПЕРЕХОДНОСТЬ:ПЕРЕХОДНЫЙ ПАДЕЖВАЛ:ВИН } : export { node:root_node THEMA_VALENCY AUX_VERB_REQUIRED ЛИЦО ЧИСЛО РОД МОДАЛЬНЫЙ }
 obj=ГлДополнение{ ПАДЕЖ:ВИН } : export {
                                         @except(v:ПАДЕЖВАЛ,ПАДЕЖ)
                                         @if_exported(ПЕРЕХОДНОСТЬ:ПЕРЕХОДНЫЙ,ПАДЕЖВАЛ)
                                        }
...
pattern СтрадСказВосх
{
 v=СтрадСказВосх : export { THEMA_VALENCY AUX_VERB_REQUIRED ЛИЦО ЧИСЛО РОД ПАДЕЖВАЛ ПЕРЕХОДНОСТЬ МОДАЛЬНЫЙ node:root_node }
 adv=Обст
}
: links { v.<ATTRIBUTE>adv }
: ngrams { adv_verb_score( adv, v ) }
...
pattern СтрадСказВосх export { THEMA_VALENCY AUX_VERB_REQUIRED ЛИЦО ЧИСЛО РОД ПАДЕЖВАЛ ПЕРЕХОДНОСТЬ (МОДАЛЬНЫЙ) node:root_node }
{
 v=СтрадСказВосх{ МОДАЛЬНЫЙ }  : export { THEMA_VALENCY AUX_VERB_REQUIRED ЛИЦО ЧИСЛО РОД ПАДЕЖВАЛ ПЕРЕХОДНОСТЬ node:root_node }
 inf=Инф2
}
: links { v.<INFINITIVE>inf }
: ngrams { v_inf_score( v, inf ) }
...
pattern СтрадСказВосх
{
 v=СтрадСказВосх{ AUX_VERB_REQUIRED:1 } : export { THEMA_VALENCY AUX_VERB_REQUIRED:0 ПАДЕЖВАЛ ПЕРЕХОДНОСТЬ МОДАЛЬНЫЙ node:root_node }
...
patterns СтрадСказНисх export { THEMA_VALENCY AUX_VERB_REQUIRED ЛИЦО ЧИСЛО РОД ПАДЕЖВАЛ ПЕРЕХОДНОСТЬ МОДАЛЬНЫЙ node:root_node }

pattern СтрадСказНисх
{
 СтрадСказВосх : export { THEMA_VALENCY AUX_VERB_REQUIRED ЛИЦО ЧИСЛО РОД ПАДЕЖВАЛ ПЕРЕХОДНОСТЬ МОДАЛЬНЫЙ node:root_node }
}
...
pattern СтрадСказНисх
{
 ...
 v=СтрадСказНисх{ THEMA_VALENCY:1 =sbj:РОД =sbj:ЧИСЛО =sbj:ЛИЦО } : export { THEMA_VALENCY:0 AUX_VERB_REQUIRED ПАДЕЖВАЛ ПЕРЕХОДНОСТЬ МОДАЛЬНЫЙ node:root_node }
} : links { v.<SUBJECT>sbj }
...
pattern СтрадСказНисх
{
 ...
 v=СтрадСказНисх{ ПЕРЕХОДНОСТЬ:ПЕРЕХОДНЫЙ =ПАДЕЖВАЛ->obj:ПАДЕЖ }
     : export {
               node:root_node THEMA_VALENCY AUX_VERB_REQUIRED РОД ЛИЦО ЧИСЛО МОДАЛЬНЫЙ
               @except(ПАДЕЖВАЛ,obj:ПАДЕЖ)
...
{
 adv=Обст
 v=СтрадСказНисх : export { THEMA_VALENCY AUX_VERB_REQUIRED ЛИЦО ЧИСЛО РОД ПАДЕЖВАЛ ПЕРЕХОДНОСТЬ МОДАЛЬНЫЙ node:root_node }
}
: links { v.<ATTRIBUTE>adv }
: ngrams { adv_verb_score( adv, v ) }
...
// Мы  уступить ему вынуждены были
//     ^^^^^^^^^^^^
pattern СтрадСказНисх export { THEMA_VALENCY AUX_VERB_REQUIRED ЛИЦО ЧИСЛО РОД ПАДЕЖВАЛ ПЕРЕХОДНОСТЬ (МОДАЛЬНЫЙ) node:root_node }
{
 inf=Инф2
 v=СтрадСказНисх{ МОДАЛЬНЫЙ }  : export { THEMA_VALENCY AUX_VERB_REQUIRED ЛИЦО ЧИСЛО РОД ПАДЕЖВАЛ ПЕРЕХОДНОСТЬ node:root_node }
}
: links { v.<INFINITIVE>inf }
: ngrams
{
...
pattern СтрадСказНисх
{
 ...
 v=СтрадСказНисх{ AUX_VERB_REQUIRED:1 =aux:РОД =aux:ЧИСЛО } : export { THEMA_VALENCY AUX_VERB_REQUIRED:0 ПАДЕЖВАЛ ПЕРЕХОДНОСТЬ МОДАЛЬНЫЙ node:root_node }
}
: links { v.<LEFT_AUX_VERB>aux }
...
{
 pn=ПредлогИСущ
 v=СтрадСказНисх{ гл_предл(_,pn.prepos,pn.n2) } : export { THEMA_VALENCY AUX_VERB_REQUIRED ЛИЦО ЧИСЛО РОД ПАДЕЖВАЛ ПЕРЕХОДНОСТЬ МОДАЛЬНЫЙ node:root_node }
}
: links { v.<PREPOS_ADJUNCT>pn }
: ngrams
{
...
{
 v=СтрадСказНисх{ THEMA_VALENCY:0 } : export { node:root_node }
}
: ngrams
{
...
pattern Сказуемое0
{
 СтрадСказНисх{ THEMA_VALENCY:1 }:export { РОД ЧИСЛО ЛИЦО:3 ВРЕМЯ:НАСТОЯЩЕЕ node:root_node }
}
