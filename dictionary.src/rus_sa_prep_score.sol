...
VerbPrep(стоять,"впритык к",1) // Шкаф стоит впритык к столу.
VerbPrep(стоять,"вплотную к",1) // Шкаф стоит вплотную к столу.
...
VerbPrep(бежать,"следом за",2) // она как раз начала бежать следом за ним
VerbPrep(быть,"рядом с",2) // он был рядом с дверью
VerbPrep(пойти,"следом за",2) // Катя пошла следом за ними.
...
VerbPrep(положить,"слева от",2) // Положи книгу слева от тетради
...
VerbPrep(хотеть,"незадолго до",1) // коллеги хотели купить торт незадолго до праздника
...
//      ^^^^^^^        ^^^^^^^^
VerbPrep(положить,"вместе с",1)
VerbPrep(ложить,"вместе с",1)
...
//   ^^^^^^^          ^^^^^^^^^^^^^^
VerbPrep(приехать,"одновременно с",1)
VerbPrep(прийти,"одновременно с",1)
VerbPrep(прибыть,"одновременно с",1)
VerbPrep(появиться,"одновременно с",1)
...
VerbPrep(глядеть,"прямо в",5) // глядя прямо в ее круглые карие глаза, произнес
VerbPrep(посмотреть,"прямо в",5)
VerbPrep(смотреть,"прямо в",5)
VerbPrep(заглянуть,"прямо в",5)
VerbPrep(швырнуть,"прямо в",5) // Он подлетает к башне и... сейчас кибер швырнет нашего  гладиатора прямо в ее пустую утробу.
VerbPrep(швырять,"прямо в",5)
VerbPrep(кинуть,"прямо в",5)
VerbPrep(кидать,"прямо в",5)
