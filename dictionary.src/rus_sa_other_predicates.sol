...
/*
// Модальная связка прошедшего времени:
// Водитель мог быть лишен прав
pattern Связка export { node:root_node ВРЕМЯ (ЛИЦО) ЧИСЛО РОД }
{
 a=глагол:*{ МОДАЛЬНЫЙ ВРЕМЯ:ПРОШЕДШЕЕ } : export { node:root_node ВРЕМЯ:ПРОШЕДШЕЕ РОД ЧИСЛО }
 be=инфинитив:быть{}
} : links { a.<INFINITIVE>be }
*/
...
// --------------------------------------------------------
// Форма БЫЛО используется в особых "безличных" оборотах:
// ей было приятно услышать это.
// --------------------------------------------------------

patterns Было_Безлич export { node:root_node }
...
// интересно было бы изучить людей
pattern Было_Безлич
{
 v='было'{ class:глагол } : export { node:root_node }
 p=ЧастицаБы
...
// Форма будущего времени также может участвовать в безличных конструкциях:
// однако вам будет очень непросто избавиться от меня.
pattern Было_Безлич
{
...
pattern ПредикатСвязка
{
 ...
 v=Было_Безлич : export { node:root_node }
 r=Обст
} : links
{
 v.{
...
// Счетное наречие в роли ремы:
// охранников было слишком много
pattern ПредикатСвязка
{
 obj=ГлДополнение{ ПАДЕЖ:РОД }
 v=Было_Безлич : export { node:root_node }
 adv=ГруппаНареч1{ НаречДляРод0 }
} : links
{
 v.{
    <OBJECT>obj
...
// Безличная конструкция со счетными наречиями:
// их было слишком много
pattern ПредикатСвязка
{
 sbj=БезличПодлежащее{ ПАДЕЖ:ДАТ }
 v=Было_Безлич : export { node:root_node }
 r=ГруппаНареч1{ НаречДляРод0 }
} : links
{
 v.{
    <SUBJECT>sbj
...
// добраться сюда им было нелегко
pattern ПредикатСвязка
{
 inf=Инф2
 sbj=БезличПодлежащее{ ПАДЕЖ:ДАТ }
...
// ему некогда было подумать об этом
pattern ПредикатСвязка
{
 sbj=БезличПодлежащее{ ПАДЕЖ:ДАТ }
 r=Обст
...
// следующей задачей было найти подходящего мага
pattern ПредикатСвязка
{
 thema=ГруппаСущ4{ ПАДЕЖ:ТВОР }
 v=Было_Безлич : export { node:root_node }
 inf=Инф2
} : links
{
...
// Полное отсутствие подлежащего:
// было приятно ощущать во рту пищу
pattern ПредикатСвязка
{
 v=Было_Безлич : export { node:root_node }
 r=Обст
...
// каждую такую страницу было приятно читать
pattern ПредикатСвязка
{
 obj=ГлДополнение
 v=Было_Безлич : export { node:root_node }
...
// приятно было освободиться от лишнего веса
pattern ПредикатСвязка
{
 r=Обст
 v=Было_Безлич : export { node:root_node }
...
// иметь эту информацию ему было незачем
pattern ПредикатСвязка
{
 inf=Инф2
 sbj=БезличПодлежащее{ ПАДЕЖ:ДАТ }
...
pattern ПредикатСвязка
{
 adv1=@optional(ПредлогИСущ)
 a=прилагательное:*{ краткий род:ср }
 v=Связка{ ВРЕМЯ:ПРОШЕДШЕЕ ЧИСЛО:ЕД РОД:СР } : export { node:root_node }
 comma=','
 conj=союз:как{}
 v2=Предикат
} : links
...
// Тоскливо было в пустых комнатах.
pattern ГлПредикат
{
 adv1=ГруппаНареч2
 v=Было_Безлич: export { node:root_node }
...
// -------------------------------
// На выставке было много народу.
// -------------------------------

wordentry_set МодифДляМного=наречие:{ очень, слишком, чересчур, чрезвычайно, запредельно, чрезмерно }

patterns КолвоОбъектовМНОГО export { node:root_node }

pattern КолвоОбъектовМНОГО
...
pattern ГлПредикат
{
 adv1=ПредлогИСущ
 v=Было_Безлич: export { node:root_node }
 c1=@optional(МодифДляМного)
 c2=НаречДляРод
 n=КолвоОбъектовМНОГО
} : links
{
 v.{
    <ATTRIBUTE>adv1
    <THEMA>n.<ATTRIBUTE>c2.~<ATTRIBUTE>c1
   }
}


// Там было мало народу.
pattern ГлПредикат
{
 adv1=ГруппаНареч2
 v=Было_Безлич: export { node:root_node }
...
// У него не было книги.
pattern ГлПредикат
{
 prep=предлог:у{}
 n1=ГлДополнение{ падеж:род }
 adv=@optional(наречие:*{})
 neg=частица:не{}
 v=Было_Безлич: export { node:root_node }
 obj=ГлДополнение{ падеж:род }
} : links
{
 v.{
    <NEGATION_PARTICLE>neg
    ~<ATTRIBUTE>adv
    <ATTRIBUTE>prep.<OBJECT>n1
    <OBJECT>obj
   }
}
...
// девочке будет хорошо у старика.
pattern ГлПредикат
{
 sbj=ГлДополнение{ падеж:дат }
 adv1=@coalesce(Обст)
...
// нашим детям будет у эльфов хорошо.
pattern ГлПредикат
{
 sbj=ГлДополнение{ падеж:дат }
 v=Было_Безлич: export { node:root_node }
...
// Неопределенная форма глагола в качестве подлежащего
wordentry_set СказуемоеДляИнф = {
 глагол:становиться{},
 глагол:стать{},
 глагол:быть{}
}
...
patterns СказуемоеДляИнфПодлеж export { node:root_node ВРЕМЯ ЧИСЛО РОД ЛИЦО }
pattern СказуемоеДляИнфПодлеж
{
 СказуемоеДляИнф : export { node:root_node ВРЕМЯ ЧИСЛО РОД ЛИЦО }
}

// отказаться было бы труднее
//            ^^^^^^^
pattern СказуемоеДляИнфПодлеж
{
 v=СказуемоеДляИнф : export { node:root_node ВРЕМЯ ЧИСЛО РОД ЛИЦО }
 p=ЧастицаБы
}


patterns НаречДляСказДляИнф export { node:root_node }

pattern НаречДляСказДляИнф { ГруппаНареч2 : export { node:root_node } }
pattern НаречДляСказДляИнф { ГруппаСрНареч2 : export { node:root_node } }

// Грести стало труднее
pattern ГлПредикат
{
 inf=Инф2
 v=СказуемоеДляИнфПодлеж{ время:прошедшее число:ед род:ср } : export { node:root_node }
 adv=@optional(НаречДляСказДляИнф)
} : links
{
 v.{
    <INFINITIVE>inf
...
// отказаться было бы действительно труднее
pattern ГлПредикат
{
 inf=Инф2
 v=СказуемоеДляИнфПодлеж{ время:прошедшее число:ед род:ср } : export { node:root_node }
 adv1=НаречДляСказДляИнф
 adv2=Обст
} : links
{
 v.{
...
// Разглядеть груз было невозможно из-за наваленных сверху автопокрышек
pattern ГлПредикат
{
 inf=Инф2
 v=СказуемоеДляИнфПодлеж{ время:прошедшее число:ед род:ср } : export { node:root_node }
 adv=НаречДляСказДляИнф
...
// Грести будет немного легче, но скучнее
pattern ГлПредикат
{
 inf=Инф2
 v=СказуемоеДляИнфПодлеж{ число:ед лицо:3 } : export { node:root_node }
 adv=@optional(НаречДляСказДляИнф)
} : links
{
 v.{
...
// *******************************************
// обратный порядок слов со словом 'нельзя'
// *******************************************
// списывать коммунальные долги нельзя
pattern ГлПредикат
{
 inf=Инф2
 v='нельзя':export { node:root_node }
} : links { v.<INFINITIVE>inf }
...
// жить здесь будет нельзя
pattern ГлПредикат
{
 inf=Инф2
 aux='будет'{ class:глагол }
 v='нельзя':export { node:root_node }
} : links
{
 v.{
...
// жить там было нельзя
pattern ГлПредикат
{
 inf=Инф2
 aux='было'{ class:глагол }
 v='нельзя':export { node:root_node }
} : links
{
 v.{
