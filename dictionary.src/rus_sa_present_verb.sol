...
pattern ГлНастМодальн_Восх
{
 ...
 obj=ГлОбъект{ [-10]=v:ПАДЕЖ } : export {
...
pattern ГлНастИнфВосх_Сказ
{
 v=ГлНастИнфВосх_Сказ{ [-1]ПЕРЕХОДНОСТЬ:ПЕРЕХОДНЫЙ } : export { node:root_node AUX_VERB_REQUIRED ЛИЦО ЧИСЛО РОД ВРЕМЯ ПАДЕЖВАЛ НАКЛОНЕНИЕ KEYFEATURE_DETECTED KEYFEATURE_DESIRABLE МОДАЛЬНЫЙ }
 obj=ГлОбъект { [-10]=v:ПАДЕЖ } : export {
                                     @except(v:ПАДЕЖ,ПАДЕЖ)
                                     @if_exported(ПЕРЕХОДНОСТЬ:ПЕРЕХОДНЫЙ,ПАДЕЖ)
                                    }
...
{
 obj=ГлОбъект
 v=ГлНастИнфНисх_Сказ{ [-1]ПЕРЕХОДНОСТЬ:ПЕРЕХОДНЫЙ [-10]=Obj:ПАДЕЖ }
     : export { 
