...
{
 p=частица:все{}
 v=Гл0{ вид:несоверш ПАДЕЖ:ВИН } : export{ ВИД ВРЕМЯ ЛИЦО ЧИСЛО НАКЛОНЕНИЕ РОД МОДАЛЬНЫЙ ПЕРЕХОДНОСТЬ ПАДЕЖ node:root_node }
} : links { v.<PREFIX_PARTICLE>p }
  : ngrams { -4 }
...
{
 n=частица:во{}
 v=Гл0{ ВРЕМЯ:НАСТОЯЩЕЕ НАКЛОНЕНИЕ:ИЗЪЯВ } : export{ ВИД ВРЕМЯ ЛИЦО ЧИСЛО НАКЛОНЕНИЕ РОД МОДАЛЬНЫЙ ПЕРЕХОДНОСТЬ ПАДЕЖ node:root_node }
}
