...
pattern Подлежащее export { ЛИЦО ЧИСЛО (РОД) node:root_node }
{
 ФИО:export { ЛИЦО:3 ЧИСЛО:ЕД node:root_node }
}
...
pattern Подлежащее
{
 ЧислСущ{ ЧИСЛО:МН } : export { node:root_node ЛИЦО:3 РОД:СР ЧИСЛО:ЕД }
}
...
pattern Подлежащее
{
 ГруппаЧисл : export { node:root_node ЛИЦО:3 РОД:СР ЧИСЛО:МН }
} : ngrams { -1 }
