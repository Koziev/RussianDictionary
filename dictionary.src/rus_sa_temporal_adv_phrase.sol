...
pattern ОбстФраза0
{
 p=ПрефиксНаречДляВрем : export { node:root_node }
...
pattern ОбстФраза0
{
 c=наречие:несколько{} : export { node:root_node }
 n=существительное:раз{ падеж:род }
...
// ^^^^^^^^^^^^^^^
wordentry_set АрибутВремениДляОбст=прилагательное:{ некоторый, долгий, "какой-то" }
pattern ОбстФраза0
{
 a=прилагательное:*{ АрибутВремениДляОбст падеж:вин род:ср число:ед степень:атриб ~краткий }
 n=существительное:время{ падеж:вин число:ед } : export { node:root_node }
} : links { n.<ATTRIBUTE>a }
...
pattern ОбстФраза0
{
 a=прилагательное:целый{ падеж:вин число:ед род:муж }
 n=существительное:год{ падеж:вин число:ед } : export { node:root_node }
}
: links { n.<ATTRIBUTE>a }
: ngrams { 2 }
...
{
 a=прилагательное:*{ /*ПрилДляВремениГода*/ ПАДЕЖ:ТВОР ЧИСЛО:ЕД }
 n=НазваниеВремениСуток{ ПАДЕЖ:ТВОР ЧИСЛО:ЕД =A:РОД }:export { node:root_node }
}
: links { n.<ATTRIBUTE>a }
: ngrams
{
...
// Антонио получил задание в понедельник утром
//                         ^^^^^^^^^^^^^^^^^^^
pattern ОбстФраза0
{
 p=предлог:в{} : export { node:root_node }
 n=ДеньНедели{ падеж:вин }
 adv=@optional(DateAdverb2)
} : links { p.<OBJECT>n.~<NEXT_COLLOCATION_ITEM>adv }
  : ngrams { 1 }

/*
// ты тогда как раз вошла
//          ^^^^^^^
pattern Обст
{
 a1=наречие:как{} : export { node:root_node }
 a2=наречие:раз{}
...
patterns ТокенГода { language=Russian } export { node:root_node }
...
{
 y=@regex("[[:digit:]]{3,4}")
 n=существительное:год{} : export { node:root_node }
} : links { n.<ATTRIBUTE>y }
...
collocation_set СловосочетКакНаречия=
{
 ...
 { "всю ночь" напролет },
 { "весь день" напролет } //,
 //{ , к счастью , }, // У механика, к счастью, нашлись запасные детали
 //{ , "к счастью" , } // У механика, к счастью, нашлись запасные детали
}
...
pattern ЧислоМесяц
{
 a = ПРИЛАГАТЕЛЬНОЕ:*{ СТЕПЕНЬ:АТРИБ ~КРАТКИЙ ЧИСЛО:ЕД РОД:СР ТИП_ПРИЛ:ПОРЯДК } : export { node:root_node ПАДЕЖ }
 n = НазваниеМесяца{ ЧИСЛО:ЕД ПАДЕЖ:РОД }
} : links { a.<RIGHT_GENITIVE_OBJECT>n }
: ngrams { 1 }
